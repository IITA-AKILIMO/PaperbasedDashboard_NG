  ...
  # Show modal when button is clicked.
    ...
    observeEvent(input$ok, {
      ...
      if(input$use == "Scheduled Planting"){
        shinyalert("Scheduled Planting",  "Cassava root yield depends on planting and harvest months. 
                                           This tool contains tables with advice for planting and harvest months for the scheduled planting use case. 
                                           For every state and LGA, a table is generated by planting month, indicated as a title of the table. The expected root yield in for harvest in 8, 9, 10, 11, 12, 13, 14 and 15 months after planting are given as columns in the tables.

                                           This window will automatically close once country data is loaded.....

               ",
                   type = "info", timer = 17000, size = 'm',
                   closeOnClickOutside = FALSE,
                   closeOnEsc = FALSE,
                   animation = FALSE,
                   html = FALSE,
...
#})
  ...
  observeEvent(input$use, {
    ...
    showTab(inputId = "nav", target = "Scheduled Planting Table")
      ...
      showTab(inputId = "nav", target = "Use case mapper")
      hideTab(inputId = "nav", target = "View maps side by side")
      hideTab(inputId = "nav", target = "View Table")
...
#######################################################################################
  ...
  ngstate <- readOGR(dsn=getwd(), layer="gadm36_NGA_2")
    ...
    lgaGroups2 <- input$lga_Groups
      ...
      }else if(input$use == "Fertilizer Recommendation" & unit == "ha"){
  ...
  observeEvent(input$ok, {
    ...
    if(input$use == "Scheduled Planting"){
        ...
        #print(head(WLY_CY_FCY1_LGA))
          ...
          # 
          # lgaGroupsSP <- "Akwa Ibom"
          # 
          # unitSP <- "acre"
    observeEvent(input$btn_SP, {
            
            #countrySP <- input$countrySP
          ...
          WLY_CY_FCY1_LGA <- WLY_CY_FCY1_LGA[WLY_CY_FCY1_LGA$NAME_1 == lgaGroupsSP, ]
      ...
      ##  NG: by Region/State and district/LGA it averages out current yield, WLY and by harvest month writes the file to the sub dir
        ...
        CY_WLY_coord_AN <- CY_WLY_coord[CY_WLY_coord$NAME_1 == lgaGroupsSP, ]
        Agg_plant_AN <- Agg_plantingMonth_SP(ds=CY_WLY_coord_AN, country = "NG", adminName = lgaGroupsSP, FCYName="FCY_1", unit)
          ...
          if(unitSP == "ha"){
            ...
            country <- "NG"
             ...
             if(plantMonthSP == "January"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(9:12, 1:4)], "plantingMonth")
            }else if(plantMonthSP == "February"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(10:12, 1:5)], "plantingMonth")
            }else if(plantMonthSP == "March"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(11:12, 1:6)], "plantingMonth")
            }else if(plantMonthSP == "April"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(12, 1:7)], "plantingMonth")
            }else if(plantMonthSP == "May"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(1:8)], "plantingMonth")
            }else if(plantMonthSP == "June"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(2:9)], "plantingMonth")
            }else if(plantMonthSP == "July"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(3:10)], "plantingMonth")
            }else if(plantMonthSP == "August"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(4:11)], "plantingMonth")
            }else if(plantMonthSP == "September"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(5:12)], "plantingMonth")
            }else if(plantMonthSP == "October"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(6:12, 1)], "plantingMonth")
            }else if(plantMonthSP == "November"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(7:12, 1:2)], "plantingMonth")
            }else if(plantMonthSP == "December"){
              colnames(SP_plDate3) <- c(switch(country, NG={rrng}, TZ={rrtz}), month.name[c(8:12, 1:3)], "plantingMonth")
            }
            ...
            output$tabletext_sp <- renderText({
              ...
              paste("Expected yield in t/acre when you plant in", plantMonthSP, "and harvest in ...", sep=" ")
            ...
            }else if(input$use == "Scheduled Planting" & input$unit_locSP == "ha"){
