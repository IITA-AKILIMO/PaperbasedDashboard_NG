  ...
  ngstate <- readOGR(dsn=getwd(), layer="gadm36_NGA_2")
    ...
    lgaGroups2 <- input$lga_Groups
      ...
      tt_dataframe <- reactive({
        ...
        df_tt <- data.frame(UreaPrice=as.numeric(input$UreaPrice),NPK151515Price=as.numeric(input$NPK151515Price),
                            CassavaPrice=as.numeric(input$CassavaPrice),
                            STATE=input$lga_Groups)
        
        # 
         #df_tt <- data.frame(UreaPrice=20000,NPK151515Price=30000,CassavaPrice=40000,
                            #STATE="Edo")
      ...
      #tt_merge <- merge(tt, df_tt,by="STATE")
      
      tt_merge$totalSalePrice = tt_merge$LGAdY  * tt_merge$CassavaPrice
      print(tt_merge$CassavaPrice)
      tt_merge$totalCost = (tt_merge$UreaPrice/50 * tt_merge$Urea) + (tt_merge$NPK151515Price/50 * tt_merge$NPK15_15_15)
      print(tt_merge$UreaPrice)
      ...
      colnames(tt_merge2) <- c("State","LGA", "Urea (kg/hectare)", "NPK 15:15:15 (kg/hectare)", "Expected yield increase (t)", 
      ...
      ttnpk <- reactive({
        ...
        if(unit == "hectare"){
          ...
          ttnpk <- paste("Recommended NPK 15:15:15 rate (kg/hectare)")
        }else {
        ...
        if (filt_select() == "Urea rate"){
          ...
          ttnpk <- reactive({
            ...
            if(unit == "hectare"){
              ...
              ttnpk <- paste("NPK 15:15:15 rate (kg/hectare)")
            }else {
      ...
      #.............................................................................................................
      #download buttons for printable guides based on unit of land and use case selection
    ...
    if(input$use == "Scheduled Planting"){
      ...
      #test parameters     
        ...
        observeEvent(input$btn_SP, {
                ...
                lgaGroupsSP <- input$lga_GroupsSP
                unitSP <- input$unit_locSP
                print(lgaGroupsSP)
                FCYName <- "FCY_1"
          
                #Filter by LGA
             CY_WLY_coord2 <- CY_WLY_coord[CY_WLY_coord$NAME_1 == lgaGroupsSP, ]
             ...
             CY_wly_average <- as.data.frame(Agg_plantingMonth_SP( ds=CY_WLY_coord2, country="NG", adminName=lgaGroupsSP, FCYName="FCY_1", unit=unitSP))
  
              SP_plDate <- CY_wly_average[CY_wly_average$plm == plantMonthSP, ]
              SP_plDate <- SP_plDate[order(SP_plDate$NAME_1, SP_plDate$NAME_2),]
              fname <- paste(lgaGroupsSP, "_SP_paperBased_ton_", plantMonthSP, ".csv",sep="")
              SP_plDate <- SP_plDate[order(SP_plDate$NAME_1, SP_plDate$NAME_2), ]
             ...
             if(plantMonthSP == "January"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(9:12, 1:4)], "plantingMonth")
            }else if(plantMonthSP == "February"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(10:12, 1:5)], "plantingMonth")
            }else if(plantMonthSP == "March"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(11:12, 1:6)], "plantingMonth")
            }else if(plantMonthSP == "April"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(12, 1:7)], "plantingMonth")
            }else if(plantMonthSP == "May"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(1:8)], "plantingMonth")
            }else if(plantMonthSP == "June"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(2:9)], "plantingMonth")
            }else if(plantMonthSP == "July"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(3:10)], "plantingMonth")
            }else if(plantMonthSP == "August"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(4:11)], "plantingMonth")
            }else if(plantMonthSP == "September"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(5:12)], "plantingMonth")
            }else if(plantMonthSP == "October"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(6:12, 1)], "plantingMonth")
            }else if(plantMonthSP == "November"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(7:12, 1:2)], "plantingMonth")
            }else if(plantMonthSP == "December"){
              colnames(SP_plDate3) <- c("STATE", "LGA", month.name[c(8:12, 1:3)], "plantingMonth")
            }
            ...
            output$SPtable <- renderDT({SP_plDate4},
                                       ...
                                       options = list(dom = 'Bfrtip',
                                                      ...
                                                      buttons = list(
                                                        list(extend = 'excel', 
                                                             filename = paste('AKILIMO SP advice', '_', lgaGroupsSP, '_', plantMonthSP),
